(function(){
'use strict';
if(window.__efAdminEnhancementsV4)return;window.__efAdminEnhancementsV4=true;
...
@media(max-width:760px){.efViewer{padding:0}.efViewerBox{border-radius:0;border-left:0;border-right:0}.efViewerHead{padding:9px}.efViewerActions .btn{padding:9px 10px}.caeWorkerDocs{grid-template-columns:1fr}.caeWorker>summary{padding:14px}.brandLogin .efLogo{width:52px;height:52px}}
`;
...
function ensureViewer(){
 ...
 viewer.querySelector('#efViewerClose').onclick=closeViewer;
...
function showViewer(url,title){
 const v=ensureViewer(),frame=v.querySelector('#efViewerFrame'),t=v.querySelector('#efViewerTitle'),dl=v.querySelector('#efViewerDownload'),ex=v.querySelector('#efViewerExternal');
 t.textContent=title||'Documento';frame.classList.remove('loaded');frame.onload=()=>frame.classList.add('loaded');frame.src=url;dl.onclick=()=>downloadUrl(url);ex.onclick=()=>nativeOpen(url,'_blank','noopener');v.classList.remove('hidden');
}
...
// Conserva el nombre del documento antes de que la app solicite la URL firmada.
document.addEventListener('click',e=>{
 const b=e.target.closest('button');if(!b)return;
 if(b.matches('[data-open]'))nextTitle=b.closest('.card')?.querySelector('.docName')?.textContent?.trim()||'Documento CAE';
 if(b.matches('[data-pay="view"]'))nextTitle=b.closest('.payCard')?.querySelector('.payName')?.textContent?.trim()||'Nómina';
 if(b.matches('[data-pay="certificate"]'))nextTitle='Certificado · '+(b.closest('.payCard')?.querySelector('.payName')?.textContent?.trim()||'Nómina');
},true);

function groupCAE(){
 const content=document.querySelector('#content');if(!content)return;
 const heading=[...content.querySelectorAll('h1,h2')].find(x=>x.textContent.trim()==='CAE / PRL');if(!heading)return;
 const scope=[...content.querySelectorAll('[data-scope]')].find(x=>x.classList.contains('primary'))?.dataset.scope;
 const grid=content.querySelector('.docGrid');if(!grid||grid.dataset.efGrouped==='1'||scope!=='employee')return;
 const cards=[...grid.children].filter(x=>x.classList.contains('card')&&!x.classList.contains('empty'));
 if(!cards.length)return;
 const groups=new Map();
 for(const card of cards){const owner=card.querySelector('.docMeta span')?.textContent?.trim()||'Sin trabajador';if(!groups.has(owner))groups.set(owner,[]);groups.get(owner).push(card)}
 const wrap=document.createElement('div');wrap.className='caeWorkerList';
 [...groups.entries()].sort((a,b)=>a[0].localeCompare(b[0],'es')).forEach(([name,docs])=>{const d=document.createElement('details');d.className='caeWorker';const s=document.createElement('summary');s.innerHTML=`<div class="caeWorkerName">${escapeHtml(name)}<small>${docs.length} documento${docs.length===1?'':'s'}</small></div>`;const body=document.createElement('div');body.className='caeWorkerDocs';docs.forEach(c=>body.appendChild(c));d.append(s,body);wrap.appendChild(d)});
 grid.dataset.efGrouped='1';grid.replaceWith(wrap);
}
function escapeHtml(s){return String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
const obs=new MutationObserver(()=>requestAnimationFrame(groupCAE));obs.observe(document.querySelector('#content'),{childList:true,subtree:true});
setTimeout(groupCAE,100);
})();