(function(){
'use strict';
if(window.__efAccountingYtdV4)return;window.__efAccountingYtdV4=true;
...
function css(){let old=document.getElementById('acYtdCss');if(old)old.remove();let s=document.createElement('style');s.id='acYtdCss';s.textContent=`
#accounting>.ac-grid:first-of-type{gap:16px!important;margin-bottom:34px!important}
#accounting>.ac-grid:first-of-type>.card{border-radius:12px!important}
#acYtd{margin-top:34px!important;border-color:#315b3a;background:linear-gradient(135deg,#0f2015,#0b160f);box-shadow:0 0 0 1px rgba(70,125,83,.08)}
#acYtd .ac-ytd-head{display:flex;justify-content:space-between;gap:14px;align-items:flex-start;margin-bottom:18px;padding-top:4px}
#acYtd .ac-ytd-title{font-size:18px;font-weight:900}.ac-ytd-sub{font-size:12px;color:var(--muted);margin-top:3px}
#acYtd .ac-ytd-kpis{display:grid;grid-template-columns:repeat(5,minmax(150px,1fr));gap:16px;margin-bottom:20px}
#acYtd .ac-ytd-kpi{border:1px solid var(--line);border-radius:12px;padding:14px;background:#0b1710}
...
#acYtd .ac-ytd-kpi b{display:block;font-size:22px;margin-top:7px;font-variant-numeric:tabular-nums}
...
#acYtd .ac-ytd-cover{margin-top:12px;font-size:12px;color:var(--muted)}#acYtd .good{color:#8ce5a4}#acYtd .bad{color:#ff9b9b}#acYtd .warn{color:#f2d46c}
#acYtd .ac-balance-note{margin:14px 0 18px;padding:12px 14px;border:1px solid #2b5135;background:#0d1c12;border-radius:12px;font-size:12px;line-height:1.55}
@media(max-width:1250px){#acYtd .ac-ytd-kpis{grid-template-columns:repeat(3,1fr)}}@media(max-width:900px){#accounting>.ac-grid:first-of-type{gap:12px!important;margin-bottom:24px!important}#acYtd{margin-top:24px!important}#acYtd .ac-ytd-kpis{grid-template-columns:1fr;gap:12px}#acYtd .table-wrap{overflow:auto}#acYtd table{min-width:600px}}
...
function render(){
 ...
 <div class="ac-ytd-kpis"><div class="ac-ytd-kpi"><span>Ingresos acumulados</span><b class="good">${fmt(t.income)}</b></div><div class="ac-ytd-kpi"><span>Gastos / salidas acumuladas</span><b class="bad">${fmt(t.expenses)}</b></div><div class="ac-ytd-kpi"><span>Beneficio / Variación neta del año</span><b class="${t.net>=0?'good':'bad'}">${fmt(t.net)}</b></div><div class="ac-ytd-kpi"><span>Saldo bancario actual detectado</span><b class="good">${lb?fmt(lb.balance):'—'}</b></div><div class="ac-ytd-kpi"><span>Saldo a principio de año</span><b class="warn">${opening!==null?fmt(opening):'—'}</b></div></div>
