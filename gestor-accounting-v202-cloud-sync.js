...
function count(v){try{return Number(v?.invoices?.length||0)+Number(v?.orders?.length||0)+Number(v?.budgets?.length||0)+Number(v?.bankMovements?.length||0)+Number(v?.importHistory?.length||0)+Number(v?.cancelledInvoices?.length||0)+Number(v?.cancelledOrders?.length||0)}catch(e){return 0}}
...
async function push(force=false){if(pushing||suppress)return;let a=data();if(!a)return;if(count(a)===0){console.info('Contabilidad cloud sync: estado local vacío, no se sube');return}let raw=stable(a),h=hash(raw);if(!force&&h===lastCloud)return;pushing=true;try{let z=await call('/desktop/state-save',{state_key:'accounting',data:a});lastCloud=h;lastObserved=h;localStorage.setItem(LS_HASH,h);localStorage.setItem(LS_TS,z.item?.updated_at||new Date().toISOString());console.info('Contabilidad sincronizada con Supabase')}catch(e){console.warn('No se pudo subir contabilidad',e)}finally{pushing=false}}
function schedulePush(){clearTimeout(pushTimer);pushTimer=setTimeout(()=>push(false),700)}
async function pull(initial=false){if(pulling||pushing)return;pulling=true;try{let z=await call('/desktop/state-get',{state_key:'accounting'}),item=z.item,a=data();if(!a)return;if(!item){await push(true);return}let cloud=item.data||{},raw=stable(cloud),h=hash(raw),localRaw=stable(a),localH=hash(localRaw),seen=localStorage.getItem(LS_TS)||'',cloudCount=count(cloud),localCount=count(a);lastCloud=h;
// Regla de recuperación: una nube vacía nunca puede borrar una contabilidad local con datos.
if(cloudCount===0&&localCount>0){console.warn('Contabilidad cloud sync: nube vacía detectada; recuperando desde este Gestor');await push(true);return}
// Una instalación vacía siempre toma la copia válida de la nube, aunque conserve marcas antiguas de sincronización.
if(localCount===0&&cloudCount>0){suppress=true;try{db.accounting=JSON.parse(raw);lastObserved=h;localStorage.setItem(LS_HASH,h);localStorage.setItem(LS_TS,item.updated_at||new Date().toISOString());let base=window.__efAccountingCloudOriginalSave||window.saveData;if(typeof base==='function')await Promise.resolve(base());try{if(typeof renderAccounting==='function'&&document.getElementById('accounting')?.getClientRects().length)renderAccounting()}catch(e){}console.info('Contabilidad recuperada desde Supabase')}finally{suppress=false}return}
if(initial&&seen===''&&localH!==h){
  // Primera activación: si el Gestor ya contiene datos, es la fuente inicial; si está vacío, toma la nube.
  if(localCount>0){await push(true);return}
}
