...
function invoicePO(i,orderSuffixes){
  ...
  if(pro){let all=invoiceStrings(i).join(' ');for(let [sf,ord] of orderSuffixes){if(new RegExp('(?:^|\\D)'+sf+'(?:\\D|$)').test(all))return ord.number}}
  return'';
}
function buildMaps(){
  let exact=new Map(),suffixMap=new Map(),suffixCounts=new Map();
  for(let o of A().orders){let p=canonFromText(o.number)||String(o.number||'').trim().toUpperCase();if(!p)continue;exact.set(p,o);let sf=suffix(p);if(sf)suffixCounts.set(sf,(suffixCounts.get(sf)||0)+1)}
  for(let o of A().orders){let p=canonFromText(o.number)||String(o.number||'').trim().toUpperCase(),sf=suffix(p);if(sf&&suffixCounts.get(sf)===1)suffixMap.set(sf,{number:p,order:o})}
  return{exact,suffixMap};
}
function matchOrder(po,maps){if(!po)return null;let c=canonFromText(po)||String(po).trim().toUpperCase();if(maps.exact.has(c))return maps.exact.get(c);let sf=suffix(c);if(sf&&maps.suffixMap.has(sf))return maps.suffixMap.get(sf).order;return null}
...
function relink(save=true){
  ...
  for(let i of a.invoices){
    let po=invoicePO(i,maps.suffixMap);if(!po){r.noPO++;continue}
    r.detected++;let canonical=canonFromText(po)||po;
    if(i.detectedOrderNumber!==canonical){i.detectedOrderNumber=canonical;changed=true}
    let o=matchOrder(canonical,maps);if(!o){r.missing++;continue}
    if(i.orderId===o.id)r.already++;else{if(i.orderId)r.corrected++;else r.linked++;i.orderId=o.id;changed=true}
    let use=consume(i);if(Number(i.orderUse)!==use){i.orderUse=use;changed=true}
  }
  if(changed&&save){try{Promise.resolve(saveData()).catch(()=>{})}catch(e){}}
  r.changed=changed;return r;
}
window.acRelinkAllOrders=function(){let r=relink(true);try{renderAccounting()}catch(e){}alert(`Asociación de pedidos revisada.\n\nFacturas totales: ${r.total}\nPedido detectado: ${r.detected}\nAsociadas ahora: ${r.linked}\nCorregidas: ${r.corrected}\nYa estaban bien: ${r.already}\nPedido no cargado todavía: ${r.missing}\nSin pedido detectable: ${r.noPO}`)};
function decorate(){
