...
function A(){try{db.accounting=db.accounting||{};let a=db.accounting;a.invoices=a.invoices||[];a.orders=a.orders||[];a.bankMovements=a.bankMovements||[];a.cancelledInvoices=a.cancelledInvoices||[];return a}catch(e){return{invoices:[],orders:[],bankMovements:[],cancelledInvoices:[]}}}
...
function fixCancelledPaidState(){let a=A(),changed=false;for(let i of a.invoices){if(!i?.cancelled)continue;if(Object.prototype.hasOwnProperty.call(i,'cancelledPreviousPaid')){let p=!!i.cancelledPreviousPaid;if(i.paid!==p){i.paid=p;changed=true}if(!p&&i.paidAt){i.paidAt=null;changed=true}}if(Object.prototype.hasOwnProperty.call(i,'cancelledPreviousRetentionPaid')){let p=!!i.cancelledPreviousRetentionPaid;if(i.retentionPaid!==p){i.retentionPaid=p;changed=true}if(!p&&i.retentionPaidAt){i.retentionPaidAt=null;changed=true}}}
 for(let i of a.cancelledInvoices){if(!i?.cancelled)continue;if(Object.prototype.hasOwnProperty.call(i,'cancelledPreviousPaid')){let p=!!i.cancelledPreviousPaid;if(i.paid!==p){i.paid=p;changed=true}if(!p&&i.paidAt){i.paidAt=null;changed=true}}}
 return changed}
...
function hookSave(){if(typeof window.acSavePdfBatch!=='function'||window.acSavePdfBatch.__v194)return;let old=window.acSavePdfBatch;let fn=async function(type){let nums=selectedNumbers(type);window.__acImportLock=true;window.__acImportRenderPending=false;let result;try{result=await old.apply(this,arguments);await new Promise(r=>setTimeout(r,450));let changed=fixCancelledPaidState();if(type==='invoice')changed=markReimports(nums)||changed;if(changed){try{await saveData()}catch(e){console.warn('Guardar estabilización importación',e)}}}finally{window.__acImportLock=false;window.__acImportRenderPending=false;try{if(typeof window.renderAccounting==='function')window.renderAccounting()}catch(e){console.warn('Render final importación',e)}}return result};fn.__v194=true;window.acSavePdfBatch=fn;try{acSavePdfBatch=fn}catch(e){}}
